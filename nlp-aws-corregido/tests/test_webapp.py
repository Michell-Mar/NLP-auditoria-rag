"""Tests for webapp/server.py. No real AWS calls, no real OpenAI calls."""
...
spec = importlib.util.spec_from_file_location("webapp_server", ROOT / "webapp/server.py")
...
def test_full_success_flow_and_download(client, monkeypatch):
    ...
    descarga_md = client.get(f"/api/audits/{job_id}/descarga/md")
    assert descarga_md.status_code == 200
    assert descarga_md.headers["content-type"].startswith("text/markdown")
...
def test_revision_recomputes_summary_and_markdown(client):
    resultado_lambda = {"state": "SUCCEEDED", "result_prefix": "results/job/"}
    server.AWS_CTX = FakeAWSContext(FakeS3(_informe_exitoso()), FakeLambda(resultado_lambda))
    job_id = client.post("/api/audits", files={"pdf": ("x.pdf", b"%PDF-1.4", "application/pdf")}).json()["job_id"]
    ...
    md = client.get(f"/api/audits/{job_id}/descarga/md").text
    assert "CUMPLE PARCIALMENTE" in md
    assert "Falta el procedimiento" in md
...
def test_saving_a_review_does_not_duplicate_the_outdated_authority_note(client):
    ...
    after "Guardar cambios de revisión". Root cause: _recalcula_resumen
    deduplicated notes by exact string match, so the nine rules' slightly
    different phrasings of the same INAI/IFAI sentence all survived.
    """
    resultado_lambda = {"state": "SUCCEEDED", "result_prefix": "results/job/"}
    ...
    assert informe_tras_revision["resumen"]["aviso_desactualizado"] is True
...
def test_recompute_logic_matches_app_audito_rag_pdf():
    """
    webapp/server.py deliberately duplicates genera_resumen/veredicto_efectivo
